...
| D-005 | Production database hosting | `DECISION_PENDING` for production data operation; operator call `ACCEPT_NEON_PREFERRED` (2026-09-25) — Neon is preferred Postgres (synthetic STAGING already; Scale + org BAA + project HIPAA enablement if counsel later requires a BAA path). See [OPERATOR_CALLS_2026-09-25.md](OPERATOR_CALLS_2026-09-25.md). Does not authorize production Veteran data. |
| D-006 | Legal/HIPAA classification | `DECISION_PENDING`; `HIPAA_APPLICABILITY = DECISION_PENDING`. Counsel packet: [D-006_FACT_SHEET.md](D-006_FACT_SHEET.md). Neon HIPAA-eligible DB controls are recorded as a **partial infrastructure mitigation** only — they do not close this decision or authorize a “HIPAA compliant” claim. |
...
| D-036 | Authentication network-signal rate-limit threshold, trusted-address policy, and privacy semantics | `DECIDED` 2026-09-25 Option **C** (destination/account limits only; no network budget) — [D036_AUTH_NETWORK_RATE_LIMIT.md](D036_AUTH_NETWORK_RATE_LIMIT.md), [OPERATOR_CALLS_2026-09-25.md](OPERATOR_CALLS_2026-09-25.md). Do not claim network throttling is implemented. |
| D-037 | Organizational funding readiness and evidence-reuse overlay (`SUAS-FUNDING-READINESS-001`) | `DECIDED` 2026-09-17: overlay `ACCEPT_AS_SPECIFIED` ([PR #25](https://github.com/scrimshawlife-ctrl/suas-specs/pull/25)); no limited-implementation authority; D-010 remains open. Packet: [D037_INDEX.md](D037_INDEX.md). Not a stack bump. No runtime, spending, eligibility, or grant-application authority. |

**D-017 expansion review:** owner-reported Lyft Business approval opened [D017_LYFT_EXPANSION_PACKET.md](D017_LYFT_EXPANSION_PACKET.md). Owner call `DEFER` (2026-09-25); it does not alter the released Uber-only adapter authority or enable Lyft credentials/effects.

D-006 remains `DECISION_PENDING`. [D-006_FACT_SHEET.md](D-006_FACT_SHEET.md) is a counsel register packet of product facts. It is not a legal opinion and does not close D-006 or set `HIPAA_APPLICABILITY`. Operator-asserted Neon Postgres HIPAA console/BAA capability is a vendor control for the DB plane ([D-006_FACT_SHEET.md](D-006_FACT_SHEET.md) §3.A); it does not classify SUAS as CE/BA/neither.

D-035 is settled by v0.5.0 as an optional status-only VA sandbox capability. D-016 remains the permitted fallback. The settlement adds no production, reporting, deletion, export, purge, or real-world-effects authority. [D035_ASSIMILATION.md](D035_ASSIMILATION.md) remains proposed reconciliation outside the released boundary. D-036 is `DECIDED` Option C (destination limits only); a later decision may select A/B with evidence.
